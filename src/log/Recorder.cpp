...
//===============================
VideoDataWriterThread::VideoDataWriterThread(){};
void VideoDataWriterThread::setup(string filePath, lockFreeQueue<ofPixels *> * q){
...
void VideoDataWriterThread::threadedFunction(){
    if(fd == -1){
...
void VideoDataWriterThread::signal(){
    condition.signal();
}

//===============================
AudioDataWriterThread::AudioDataWriterThread(){};
void AudioDataWriterThread::setup(string filePath, lockFreeQueue<audioFrameShort *> *q){
    this->filePath = filePath;
    fd = -1;
    queue = q;
    bIsWriting = false;
...
void AudioDataWriterThread::threadedFunction(){
    if(fd == -1){
        fd = ::open(filePath.c_str(), O_WRONLY);
    }
...
void AudioDataWriterThread::signal(){
    condition.signal();
}
...
bool Recorder::setupCustomOutput(int w, int h, float fps, string outputString){
    return setupCustomOutput(w, h, fps, 0, 0, outputString);
}

bool Recorder::setupCustomOutput(int w, int h, float fps, int sampleRate, int channels, string outputString)
{
    if(bIsInitialized)
    {
        close();
    ...
    if(!bRecordVideo && !bRecordAudio) {
        ofLogWarning() << "Recorder::setupCustomOutput(): invalid parameters, could not setup video or audio stream.\n"
        << "video: " << w << "x" << h << "@" << fps << "fps\n"
    ...
    if(bIsInitialized && bRecordVideo)
    {
        ...
        if(bRecordAudio && !bFinishing){
            ...
            if(avDelta > 1.0/frameRate) {
                ...
                ofLogVerbose() << "Recorder: avDelta = " << avDelta << ". Not enough video frames for desired frame rate, copied this frame " << framesToAdd << " times.\n";
            }
            else if(avDelta < -1.0/frameRate){
                //more than one video frame is waiting, skip this frame
                framesToAdd = 0;
                ofLogVerbose() << "Recorder: avDelta = " << avDelta << ". Too many video frames, skipping.\n";
            }
        }
...
void Recorder::addAudioSamples(float *samples, int bufferSize, int numChannels){
...
set<int> Recorder::openPipes;

int Recorder::requestPipeNumber(){
    int n = 0;
...
void Recorder::retirePipeNumber(int num){
    if(!openPipes.erase(num)){
        ofLogNotice() << "Recorder::retirePipeNumber(): trying to retire a pipe number that is not being tracked: " << num << endl;
    }
}
